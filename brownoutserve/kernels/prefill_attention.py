...
def _fwd_prefill_attention(
    o: torch.Tensor,	# [num_prefill_tokens, n_heads, head_dim]
    q: torch.Tensor,	# [num_prefill_tokens, n_heads, head_dim]
    k: torch.Tensor,	# [num_prefill_tokens, n_kv_heads, head_dim]
    v: torch.Tensor,	# [num_prefill_tokens, n_kv_heads, head_dim]
    softmax_scale: float,
    ...
    q += (my_q_start_loc*n_heads+my_q_head)*head_dim
    k += (my_q_start_loc*n_kv_heads+my_kv_head)*head_dim
    v += (my_q_start_loc*n_kv_heads+my_kv_head)*head_dim
    o += (my_q_start_loc*n_heads+my_q_head)*head_dim

    range_my_q = my_q_block*BLOCK_Q + tl.arange(0, BLOCK_Q)
    offs_my_q = range_my_q[:, None]*(n_heads*head_dim) + tl.arange(0, head_dim)[None, :]
    my_q = tl.load(q + offs_my_q, mask = range_my_q[:, None] < my_seq_len, cache_modifier=".cg") # [BLOCK_Q, head_dim]

    k_ptrs = k + (tl.arange(0, BLOCK_K))[None, :]*(n_kv_heads*head_dim) + tl.arange(0, head_dim)[:, None]
    v_ptrs = v + (tl.arange(0, BLOCK_K))[:, None]*(n_kv_heads*head_dim) + tl.arange(0, head_dim)[None, :]
    ...
    for k_block_start in range(0, my_q_block*BLOCK_Q, BLOCK_K):
        ...
        cur_k = tl.load(k_ptrs + k_block_start*(n_kv_heads*head_dim), cache_modifier=".cg") # [head_dim, BLOCK_K]
        qk = tl.dot(my_q, cur_k, out_dtype=tl.float32) * softmax_scale # [BLOCK_Q, BLOCK_K]
        cur_k = None
        cur_v = tl.load(v_ptrs + k_block_start*(n_kv_heads*head_dim), cache_modifier=".cg") # [BLOCK_K, head_dim]
    ...
    for k_block_start in range(my_q_block*BLOCK_Q, (my_q_block+1)*BLOCK_Q, BLOCK_K):
        k_block_start = tl.multiple_of(k_block_start, BLOCK_K)
        cur_k = tl.load(k_ptrs + k_block_start*(n_kv_heads*head_dim),
                        mask = (k_block_start + tl.arange(0, BLOCK_K))[None, :] < my_seq_len,
                        cache_modifier=".cg")    # [head_dim, BLOCK_K]
        qk = tl.dot(my_q, cur_k, out_dtype=tl.float32) * softmax_scale  # [BLOCK_Q, BLOCK_K]
        cur_k = None
        cur_v = tl.load(v_ptrs + k_block_start*(n_kv_heads*head_dim),
                        mask = (k_block_start + tl.arange(0, BLOCK_K))[:, None] < my_seq_len,
...
def prefill_attention(
    ...
    k: torch.Tensor,    # [num_prefill_tokens, n_kv_heads, head_dim]
    v: torch.Tensor,    # [num_prefill_tokens, n_kv_heads, head_dim]
    o: torch.Tensor,    # [num_prefill_tokens, n_q_heads, head_dim]
    ...
    grid = (infer_state.num_prefill_seqs, model_config.n_heads, triton.cdiv(infer_state.max_prefill_len, BLOCK_Q))
    num_warps = 8
